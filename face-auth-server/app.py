...
# # Store previous face positions for movement detection
# face_positions_cache = {}
...
#         # Get the face location (top, right, bottom, left)
...
#         # Check if we have a previous position for this session
#         movement = 0
#         movement_detected = False
        
#         if session_id in face_positions_cache:
#             prev_center = face_positions_cache[session_id]
            
#             # Calculate Euclidean distance between current and previous centers
#             dx = face_center['x'] - prev_center['x']
#             dy = face_center['y'] - prev_center['y']
            
#             # Normalize by face size to account for distance from camera
#             face_size = (face_center['width'] + face_center['height']) / 2
#             movement = np.sqrt(dx*dx + dy*dy) / face_size * 100
            
#             # Threshold for significant movement (adjust as needed)
#             threshold = 15
#             movement_detected = movement > threshold
        
#         # Update the cache with current position
#         face_positions_cache[session_id] = face_center
        
#         # Clean up old sessions (optional)
#         if len(face_positions_cache) > 1000:
#             # Remove oldest entries if cache gets too large
#             oldest_keys = sorted(face_positions_cache.keys())[:100]
#             for key in oldest_keys:
#                 if key != session_id:
#                     face_positions_cache.pop(key, None)
